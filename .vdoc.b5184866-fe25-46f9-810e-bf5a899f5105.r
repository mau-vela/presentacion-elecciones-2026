...
mapa_dep <- function(var, titulo, es_cambio) {
  vals <- mp[[var]]
  if (es_cambio) {
    pal <- colorBin(c("#cb181d", "#f2b0a8", "#f7f7f7", "#9ecae1", "#2171b5"),
                    domain = vals, bins = c(-Inf, -5, -1, 1, 5, Inf), na.color = "#eeeeee")
  } else {
  ...
  leaflet(mp, height = 540) %>% addProviderTiles("CartoDB.Positron") %>%
    addPolygons(weight = 0.6, color = "#ffffff", fillColor = pal(vals), fillOpacity = 0.88,
                ...
                highlightOptions = highlightOptions(weight = 2, color = "#0B2E63", bringToFront = TRUE)) %>%
    { if (es_cambio) {
        addLegend(., colors = c("#cb181d", "#f2b0a8", "#f7f7f7", "#9ecae1", "#2171b5"),
                  labels = c("menor a −5 pp", "−5 a −1 pp", "−1 a 1 pp", "1 a 5 pp", "mayor a 5 pp"),
