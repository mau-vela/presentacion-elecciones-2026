...
#
#| label: calc-kpi-ext
censo_26_val <- datos_ext$censo_ext %>% filter(annoh == 2026) %>% pull(censo)
part_sen_26 <- datos_ext$part_ext_congreso %>% filter(annoh == 2026, corp == "Senado") %>% pull(part_pct)
part_sen_22 <- datos_ext$part_ext_congreso %>% filter(annoh == 2022, corp == "Senado") %>% pull(part_pct)
#
#
#
#
