...
mapa_dep <- function(var, titulo, es_cambio) {
  ...
  if (es_cambio) {
    ...
    fmt_v <- function(x) sprintf("%s pp", formatC(x, 1, format = "f"))
  } else {
    pal <- colorNumeric("Blues", domain = vals, na.color = "#eeeeee")
    fmt_v <- function(x) sprintf("%s%%", formatC(x, 1, format = "f"))
  }
  ...
  leaflet(mp, height = 540) %>% addProviderTiles("CartoDB.Positron") %>%
    addPolygons(weight = 0.6, color = "#ffffff", fillColor = pal(vals), fillOpacity = 0.88,
                ...
                highlightOptions = highlightOptions(weight = 2, color = "#0B2E63", bringToFront = TRUE)) %>%
    { if (es_cambio) {
        cortes <- c(-Inf, -5, -1, 1, 5, Inf)
        pal_bin <- colorBin(c("#cb181d", "#f2b0a8", "#f7f7f7", "#9ecae1", "#2171b5"),
                            domain = vals, bins = cortes, na.color = "#eeeeee")
        clearControls(.) %>%
          addLegend(pal = pal_bin, values = vals, title = titulo, position = "bottomright",
                    opacity = 0.9,
