...
#
#
#| label: mapa-esp-funciones
sf_mpios_esp <- datos_part$sf_mpios %>% mutate(codmpio = as.integer(codmpio)) %>% st_transform(4326)
sf_deptos_esp <- datos_part$sf_deptos %>% mutate(coddepto = as.integer(coddepto)) %>% st_transform(4326)
sf_citrep_esp <- datos_esp$sf_citrep %>% mutate(CTEP = as.integer(CTEP)) %>% st_transform(4326)

crear_mapa_esp <- function(elec_sel, nivel_sel, crit_sel) {
  if (nivel_sel == "Regional") {
    d <- datos_esp$regional %>% filter(eleccion == elec_sel)
    if (elec_sel == "CITREP") {
      mp <- sf_citrep_esp %>% left_join(d, by = c("CTEP" = "id_region"))
      lab_nombre <- mp$nombre_region
    } else {
      mp <- sf_deptos_esp %>% left_join(d, by = c("coddepto" = "id_region"))
      lab_nombre <- mp$nombre_region
    }
  } else {
    d <- datos_esp$municipal %>% filter(eleccion == elec_sel)
    mp <- sf_mpios_esp %>% left_join(d, by = "codmpio")
    lab_nombre <- mp$Municipio
  }
  
  if (crit_sel == "Nivel 2026") {
    var <- "part_2026"
    # Uso de colorBin para distribuir equitativamente los tonos de azul y evitar que quede todo igual
    pal <- colorBin("Blues", domain = mp[[var]], bins = 5, na.color = "#eeeeee")
    titulo <- "Participación 2026 (%)"; suf <- "%"
  } else if (crit_sel == "Cambio vs 2022") {
    var <- "cambio_22"; m <- max(abs(mp[[var]]), na.rm = TRUE)
    pal <- colorNumeric(c("#cb181d", "#f7f7f7", "#2171b5"), domain = c(-m, m), na.color = "#eeeeee")
    titulo <- "Cambio vs 2022 (pp)"; suf <- " pp"
  } else {
    var <- "cambio_18"; m <- max(abs(mp[[var]]), na.rm = TRUE)
    pal <- colorNumeric(c("#cb181d", "#f7f7f7", "#2171b5"), domain = c(-m, m), na.color = "#eeeeee")
    titulo <- "Cambio vs 2018 (pp)"; suf <- " pp"
  }
  
  vals <- mp[[var]]
  
  if (elec_sel == "CITREP") {
    lab <- sprintf("<b>%s</b><br>2026: %s%%<br>vs 2022: %s pp",
      lab_nombre, formatC(mp$part_2026, 1, format = "f", decimal.mark = ","),
      formatC(mp$cambio_22, 1, format = "f", decimal.mark = ",")) %>% lapply(htmltools::HTML)
  } else {
    lab <- sprintf("<b>%s</b><br>2026: %s%%<br>vs 2022: %s pp &nbsp;·&nbsp; vs 2018: %s pp",
      lab_nombre, formatC(mp$part_2026, 1, format = "f", decimal.mark = ","),
      formatC(mp$cambio_22, 1, format = "f", decimal.mark = ","),
      formatC(mp$cambio_18, 1, format = "f", decimal.mark = ",")) %>% lapply(htmltools::HTML)
  }
  
  leaflet(mp, height = 540) %>% addProviderTiles("CartoDB.Positron") %>%
    addPolygons(weight = 0.3, color = "#ffffff", fillColor = pal(vals), fillOpacity = 0.9, label = lab,
                highlightOptions = highlightOptions(weight = 1.5, color = "#0B2E63", bringToFront = TRUE)) %>%
    addLegend(pal = pal, values = vals, title = titulo, position = "bottomleft", labFormat = labelFormat(suffix = suf))
}
#
#
#
...
#
#
crear_mapa_esp("CITREP", "Regional", "Nivel 2026")
```
#
#
#
crear_mapa_esp("CITREP", "Regional", "Cambio vs 2022")
```
#
#
#
crear_mapa_esp("CITREP", "Municipal", "Nivel 2026")
```
#
#
#
crear_mapa_esp("CITREP", "Municipal", "Cambio vs 2022")
```
#
#
#
#
crear_mapa_esp("Cámara Afro", "Regional", "Nivel 2026")
```
#
#
#
crear_mapa_esp("Cámara Afro", "Regional", "Cambio vs 2022")
```
#
#
#
crear_mapa_esp("Cámara Afro", "Regional", "Cambio vs 2018")
```
#
#
#
crear_mapa_esp("Cámara Afro", "Municipal", "Nivel 2026")
```
#
#
#
crear_mapa_esp("Cámara Afro", "Municipal", "Cambio vs 2022")
```
#
#
#
crear_mapa_esp("Cámara Afro", "Municipal", "Cambio vs 2018")
```
#
#
#
#
crear_mapa_esp("Senado Indígena", "Regional", "Nivel 2026")
```
#
#
#
crear_mapa_esp("Senado Indígena", "Regional", "Cambio vs 2022")
```
#
#
#
crear_mapa_esp("Senado Indígena", "Regional", "Cambio vs 2018")
```
#
#
#
crear_mapa_esp("Senado Indígena", "Municipal", "Nivel 2026")
```
#
#
#
crear_mapa_esp("Senado Indígena", "Municipal", "Cambio vs 2022")
```
#
#
#
crear_mapa_esp("Senado Indígena", "Municipal", "Cambio vs 2018")
```
#
#
#
#
crear_mapa_esp("Cámara Indígena", "Regional", "Nivel 2026")
```
#
#
#
crear_mapa_esp("Cámara Indígena", "Regional", "Cambio vs 2022")
```
#
#
#
crear_mapa_esp("Cámara Indígena", "Regional", "Cambio vs 2018")
```
#
#
#
crear_mapa_esp("Cámara Indígena", "Municipal", "Nivel 2026")
```
#
#
#
crear_mapa_esp("Cámara Indígena", "Municipal", "Cambio vs 2022")
```
#
#
#
crear_mapa_esp("Cámara Indígena", "Municipal", "Cambio vs 2018")
```
#
#
#
