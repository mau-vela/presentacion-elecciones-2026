...
```{ojs}
viewof ve_tipo_nal = Inputs.radio(["Votos en blanco", "Votos nulos", "Votos no marcados"], {value: "Votos en blanco", label: html`<b style="font-size:14px; color:#1F2937;">Tipo de voto:</b>`})
```
...
viewof ve_map_tipo = Inputs.radio(["Votos en blanco", "Votos nulos", "Votos no marcados"], {value: "Votos en blanco", label: html`<b style="font-size:14px; color:#1F2937;">Tipo:</b>`})
viewof ve_map_corp = Inputs.radio(["Senado", "Cámara"], {value: "Senado", label: html`<b style="font-size:14px; color:#1F2937;">Corporación:</b>`})
viewof ve_map_nivel = Inputs.radio(["Departamental", "Municipal"], {value: "Departamental", label: html`<b style="font-size:14px; color:#1F2937;">Nivel:</b>`})
viewof ve_map_top = Inputs.select(["Top 20 más alto", "Top 20 más bajo"], {value: "Top 20 más alto"})
```

```{ojs}
html`
<div style="display: flex; gap: 32px; background: #F8FAFC; padding: 12px 20px; border-radius: 8px; border: 1px solid #E2E8F0; align-items: center; margin-bottom: 16px;">
  <div style="display: flex; align-items: center; gap: 12px;">${viewof ve_map_tipo}</div>
  <div style="display: flex; align-items: center; gap: 12px; border-left: 1px solid #CBD5E1; padding-left: 32px;">${viewof ve_map_corp}</div>
  <div style="display: flex; align-items: center; gap: 12px; border-left: 1px solid #CBD5E1; padding-left: 32px;">${viewof ve_map_nivel}</div>
</div>
`
```
...
mapa_ve_L = {
  ...
  L_ve.tileLayer("https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png",
    { subdomains: "abcd", attribution: "© OpenStreetMap © CARTO" }).addTo(map);
  invalidation.then(() => map.remove());
  setTimeout(() => map.invalidateSize(), 200);
  div.value = map;
...
```{ojs}
viewof ve_esp_elec = Inputs.radio(["CITREP", "Senado Indígena", "Cámara Indígena", "Cámara Afro"], {value: "CITREP", label: html`<b style="font-size:14px; color:#1F2937;">Elección:</b>`})
viewof ve_esp_tipo = Inputs.radio(["Votos en blanco", "Votos nulos", "Votos no marcados"], {value: "Votos en blanco", label: html`<b style="font-size:14px; color:#1F2937;">Tipo:</b>`})
```

```{ojs}
html`
<div style="display: flex; gap: 32px; background: #F8FAFC; padding: 12px 20px; border-radius: 8px; border: 1px solid #E2E8F0; align-items: center; margin-bottom: 24px;">
  <div style="display: flex; align-items: center; gap: 12px;">${viewof ve_esp_elec}</div>
  <div style="display: flex; align-items: center; gap: 12px; border-left: 1px solid #CBD5E1; padding-left: 32px;">${viewof ve_esp_tipo}</div>
</div>
`
```
...
mapa_ve_esp_L = {
  ...
  L_ve_esp.tileLayer("https://{s}.basemaps.cartocdn.com/light_all/{z}/{x}/{y}{r}.png",
    { subdomains: "abcd", attribution: "© OpenStreetMap © CARTO" }).addTo(map);
  invalidation.then(() => map.remove());
  setTimeout(() => map.invalidateSize(), 200);
  div.value = map;
...
```{ojs}
viewof el_corp = Inputs.radio(["Senado", "Cámara"], {value: "Senado", label: html`<b style="font-size:14px; color:#1F2937;">Corporación:</b>`})
viewof el_anno = Inputs.radio([2026, 2022], {value: 2026, format: x => `${x}`, label: html`<b style="font-size:14px; color:#1F2937;">Año:</b>`})
```
